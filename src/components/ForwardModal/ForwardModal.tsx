...
interface Props {
  ...
  onNavigateToChannel?: (serverId: number, chatroomId: number, chatroomName: string, serverName: string) => void;
...
export default function ForwardModal({ messageText, userId, username, nameColor, currentChatroomId, isAdmin, sourceContext, onNavigateToChannel, onNavigateToDM, onClose }: Props) {
  const socket = getSocket();
  const servers = useAppSelector(s => s.server.servers);
  const friends = useAppSelector(s => s.friend.friends).filter(f => f.friendId !== null && f.friendId !== userId);
  ...
  useEffect(() => {
    servers.forEach(server => {
      axios.get('/api/v1/chatrooms', { params: { serverId: server.serverId } })
        .then(res => {
          const textOnly = (res.data as Chatroom[]).filter(c => c.type === 'text' || c.type === 'voice');
  ...
  const forwardToChannel = (serverId: number, chatroomId: number, channelName: string, serverName: string) => {
    if (sending) return;
    setSending(true);
    socket.emit('CHATROOM_MESSAGE', {
      username, message: fullMessage, userId, chatroomId,
      room: `${BASE}/chatroom/${serverId}/${chatroomId}`, nameColor,
  ...
  const forwardToFriend = (friend: Friend) => {
    ...
    socket.emit('SEND_PRIVATE_MESSAGE', {
      username, message: fullMessage, userId,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-xl bg-gray-800 shadow-xl overflow-hidden" onClick={e => e.stopPropagation()}>
        {/* Header */}
        ...
        <>
            ...
            <div className="max-h-64 overflow-y-auto px-4 py-3 space-y-4">
              {filteredFriends.length > 0 && (
                <div>
                  <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-500">Direct Messages</p>
              ...
              {filteredServers.map(({ server, channels }) => (
                <div key={server.serverId}>
                  <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-500">{server.name}</p>
                  <div className="space-y-0.5">
                    {channels.map(ch => (
                      <button
                        key={ch.id}
