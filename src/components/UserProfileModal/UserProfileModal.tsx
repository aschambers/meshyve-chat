        ...
        <div className="px-5 pb-5">
          ...
          <div className="relative -mt-8 mb-3 inline-block">
            <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-gray-900 ring-4 ring-gray-600 text-xl font-bold text-white">
              {displayImage
                ? <img src={displayImage} alt={username} className="h-full w-full object-cover" loading="eager" />
                : username[0]?.toUpperCase()
              }
            </div>
            {status && (
              <span className={`absolute bottom-[1px] right-[1px] h-5 w-5 rounded-full border-2 border-gray-800 ${STATUS_COLOR[status]}`} />
            )}
            {isSelf && onEditProfile && (
              <div className="absolute -top-1 -right-0.5 group/pencil z-10">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-2.5 h-2.5" viewBox="0 0 24 24" fill="none" stroke="#1a1610" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex items-center justify-between gap-2 mt-1">
            ...
            {!isSelf && onSendMessage && (
              <Tooltip text="Send Message">
                <button onClick={() => { onSendMessage(); onClose(); }} className="text-gray-400 hover:text-yellow-400 transition-colors">
            ...
            {!isSelf && currentUserId && (() => {
              ...
              return (
                <Tooltip text="Add Friend">
                  <button onClick={() => { setLocalStatus('pending-sent'); onAddFriend?.(); }} className="text-gray-400 hover:text-yellow-400 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex gap-2">
            {!isSelf && currentUserId && (() => {
              ...
              if (isAccepted) return (
                <button onClick={() => { setLocalStatus('removed'); onUnfriend?.(); }} className="rounded bg-gray-600 px-3 py-1.5 text-sm text-gray-300 hover:bg-red-600 hover:text-white transition-colors">
                  Remove Friend
                </button>
              );
              if (isPendingSent) return (
                <button disabled className="rounded bg-gray-500 px-3 py-1.5 text-sm text-white cursor-not-allowed opacity-70">Pending</button>
              );
              if (isPendingIncoming) return (
                <button onClick={() => { setLocalStatus('accepted'); onAcceptRequest?.(friendRequest!.id); onClose(); }} className="rounded bg-green-600 px-3 py-1.5 text-sm text-white hover:bg-green-500">
                  Accept
                </button>
              );
              return null;
            ...
            <div className="mt-4 border-t border-gray-700 pt-3 flex items-center gap-2 flex-wrap">
              {memberSince && (
                <div className="flex items-center gap-1.5">
                  <img src="/meshyve-logo.svg" alt="Meshyve" className="h-5 w-5 rounded-full flex-shrink-0" />
              ...
              {serverJoinedAt && (
                <div className="flex items-center gap-1.5">
                  {serverImageUrl
                    ? <img src={serverImageUrl} alt={serverName} className="h-4 w-4 rounded-full object-cover flex-shrink-0" />
                    : serverName
                      ? <span className="flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full bg-gray-600 text-[0.5625rem] font-bold text-white">{serverName[0].toUpperCase()}</span>
                      : null
                  }
